...
async def init_db():
	...
	async with pool.acquire() as conn:
		...
		CREATE TABLE IF NOT EXISTS content_blocks (
			...
			-- ✅ FIX: по умолчанию delay должен быть 0.0, а не 1.0
			delay_seconds DOUBLE PRECISION DEFAULT 0.0,
		...
		for col, ddl in [
			# ✅ раньше был DEFAULT 1.0 — но это только для добавления новой колонки на старых базах
			("delay_seconds", "ALTER TABLE content_blocks ADD COLUMN delay_seconds DOUBLE PRECISION DEFAULT 0.0;"),
		...
		try:
			await conn.execute("ALTER TABLE content_blocks ALTER COLUMN delay_seconds SET DEFAULT 0.0;")
		except Exception:
			pass
	...
	return [
		{
			...
			# ✅ FIX: по умолчанию 0.0 (никаких скрытых задержек)
			"delay": float(r["delay_seconds"] or 0.0),
